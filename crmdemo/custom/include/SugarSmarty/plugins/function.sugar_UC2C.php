 ...
 * @param Smarty
 */
function smarty_function_sugar_UC2C($params, &$smarty)
{
	...
	if ( isset($params['value']) ) {
		$phone = strip_tags($params['value']);
	} else {
		$phone = '';
	}
    if(isset($system_config->settings['system_skypeout_on']) && $system_config->settings['system_skypeout_on'] == 1
    	&& isset($phone) && skype_formatted($phone)  ) {
    		$GLOBALS['log']->debug($phone);
			return '<a href="#" onclick="YAHOO.util.Connect.asyncRequest(\'GET\',\'index.php?module=uc2ce_Click2Call&action=click2call&to_pdf=true&number='.format_skype($phone).'\', callback); return false;">'.$phone.'</a>';
    } elseif(isset($_SESSION['isMobile'])) {
        return '<a href="tel:'.format_skype($phone).'">'.$phone.'</a>';
    } else {
    	return $phone;
    }
}
