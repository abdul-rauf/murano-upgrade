...
function get_deleted_lead_list($session, $module_name, $query, $order_by,$offset, $select_fields, $max_results, $deleted ){
	
	global  $beanList, $beanFiles ,$db;
 $GLOBALS['log']->fatal("works good");   
	$class_name = $beanList[$module_name];
	require_once($beanFiles[$class_name]);
	$seed = new $class_name();
	...
	$response =Array();
$list  =Array();
   // $query ="SELECT id from leads where data_updated =1 and deleted =1 order by leads.date_modified ";
    $query ="SELECT id from leads where data_updated =1 and deleted =1 and (leads.date_modified >= DATE_SUB(NOW(), INTERVAL 24 HOUR)) order by leads.date_modified ";

        $response = $seed->process_list_query($query,0,500,500);
...
$list = $response['list'];
	...
	// Calculate the offset for the start of the next page
	$next_offset = $offset + sizeof($response['list']);
...
function get_user_team_id($session){
	$GLOBALS['log']->info('Begin: SugarWebServiceImpl->get_user_team_id');
