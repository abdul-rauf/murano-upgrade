...
$cur_date = date('m/d/y');
$doc_location = "custom/modules/Leads/leads24_new.rtf";
$fp = file_get_contents($doc_location);
@ob_end_clean();
$fp =str_replace('Account_name',html_entity_decode(utf8_to_rtf_decoded(strtoupper(htmlspecialchars_decode($focus->account_name)))),$fp);
...
echo $focus->typ_invest_c;
die();
$fp =str_replace(' _typ_invest_c_',utf8_to_rtf_decoded($focus->typ_invest_c),$fp);
$fp =str_replace('_req_aum_c_',utf8_to_rtf_decoded($focus->req_aum_c),$fp);
$fp =str_replace('_min_track_c_',trim($focus->min_track_c),$fp);
