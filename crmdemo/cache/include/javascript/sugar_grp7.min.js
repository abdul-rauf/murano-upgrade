...
app.controller.loadView({module:"Styleguide",layout:layout,page_name:page,field_type:field,skipFetch:true});}},{name:"list",route:":module"},{name:"create",route:":module/create",callback:function(module){if(module==="Home"){app.controller.loadView({module:module,layout:"record"});return;}
if(!app.router._moduleExists(module)){return;}
var previousModule=app.controller.context.get("module"),previousLayout=app.controller.context.get("layout");if(!(previousModule===module&&previousLayout==="records")){app.controller.loadView({module:module,layout:"records"});}
app.drawer.open({layout:'create-actions',context:{create:true}},_.bind(function(context,model){var module=context.get("module")||model.module,route=app.router.buildRoute(module);app.router.navigate(route,{trigger:(model instanceof Backbone.Model)});},this));}},{name:"vcardImport",route:":module/vcard-import",callback:function(module){if(!app.router._moduleExists(module)){return;}
app.controller.loadView({module:module,layout:"records"});app.drawer.open({layout:'vcard-import'},_.bind(function(){var route=app.router.buildRoute(module);app.router.navigate(route,{replace:true});},this));}},{name:"layout",route:":module/layout/:view"},{name:'config',route:':module/config',callback:function(module){if(!app.router._moduleExists(module)){return;}
var previousModule=app.controller.context.get("module"),previousLayout=app.controller.context.get("layout");if(!(previousModule===module&&previousLayout==="records")){app.controller.loadView({module:module,layout:"records"});}
app.drawer.open({layout:'config',context:{module:module,create:true}},_.bind(function(context,model){var module=context.get("module")||model.module,route=app.router.buildRoute(module);app.router.navigate(route,{trigger:(model instanceof Backbone.Model)});},this));}},{name:"homeRecord",route:"Home/:id",callback:function(id){var lastHomeKey=getLastHomeKey();app.user.lastState.set(lastHomeKey,homeOptions.dashboard);app.router.record("Home",id);}},{name:"record",route:":module/:id"},{name:"record",route:":module/:id/:action"},{name:"record_layout",route:":module/:id/layout/:view",callback:function(module,id,view){if(!app.router._moduleExists(module)){return;}
...
if(showWizard){var callbacks={complete:function(){var module=app.utils.getWindowLocationParameterByName('module',window.location.search),action=app.utils.getWindowLocationParameterByName('action',window.location.search);if(_.isString(module)&&_.isString(action)&&module.toLowerCase()==='users'&&action.toLowerCase()==='authenticate'){window.location=window.location.pathname;}else{window.location.reload();}}};app.controller.loadView({layout:'first-login-wizard',module:'Users',modelId:app.user.get('id'),callbacks:callbacks,wizardName:app.user.get('type')});$('#header').hide();return false;}
var passwordExpired=false;if(route&&route!=='logout'&&app.user&&app.user.has('is_password_expired')){passwordExpired=app.user.get('is_password_expired');if(passwordExpired){app.controller.loadView({layout:'password-expired',module:'Users',callbacks:{complete:function(){window.location.reload();}},modelId:app.user.get('id')});return false;}}
var subroute;if(module){var qpos=module.indexOf('?');subroute=qpos>-1?module.substring(0,module.indexOf('?')):module;}
var viewId=options.route+(subroute?'/'+subroute:'');app.analytics.currentViewId=viewId;app.analytics.trackPageView(app.analytics.currentViewId);return true;}});app.augment("progress",_.extend({init:function(){NProgress.configure({template:'<div class="loading gate">'+'    <div class="progress progress-danger">'+'        <div role="bar" class="bar"></div>'+'    </div>'+'</div>'});NProgress.start();NProgress.set(0.25);},hide:function(){$("#nprogress").hide();}},NProgress),false);app.events.on("app:logout:success",function(data){if(app.config&&app.config.externalLogin&&data&&data.url){if(!$('#logoutframe').length){$("#sugarcrm").append('<iframe id="logoutframe" />');$('#logoutframe').hide();}
$('#logoutframe').load(function(){$('#logoutframe').off('load');$('#logoutframe').attr('src','');});$('#logoutframe').attr('src',data.url);}});})(SUGAR.App);
...
if(field.view&&field.view.trigger){field.view.trigger('field:error',field,true);}
...
return(this.isDisabled()&&viewName==='disabled')?'edit':(this.view.fallbackFieldTemplate||'detail');},_render:function(){this.destroyAllErrorTooltips();var isErrorState=this.$('.add-on.error-tooltip').length>0;this._processHelp();_fieldProto._render.call(this);if(this._previousAction){this._addViewClass(this._previousAction);}
this._addViewClass(this.action);if(isErrorState){this.clearErrorDecoration();if(this.action==='edit'){this.decorateError(this._errors);}}
...
var isWrapped=$ftag.parent().hasClass('input-append');if(!isWrapped){$ftag.wrap('<div class="input-append error '+ftag+'">');}else{$ftag.parent().addClass('error');}
$tooltip=$(this.exclamationMarkTemplate(errorMessages));$ftag.after($tooltip);this.createErrorTooltips($tooltip);},createErrorTooltips:function($element){this._errorTooltips=this._errorTooltips||[];this._errorTooltips.push(app.utils.tooltip.initialize($element));},destroyAllErrorTooltips:function(){app.utils.tooltip.destroy(this._errorTooltips);this._errorTooltips=null;},_dispose:function(){this.destroyAllErrorTooltips();_fieldProto._dispose.call(this);},clearErrorDecoration:function(){var ftag=this.fieldTag||'',$ftag=this.$(ftag);this.$('.add-on.error-tooltip').remove();var isWrapped=$ftag.parent().hasClass('input-append');var isDateField=$ftag.parent().hasClass('date');if(isDateField){$ftag.parent().removeClass('error');}else if(isWrapped){$ftag.unwrap();}
...
events['click a[href="javascript:void(0)"]']='_handleBadLinkHref';events['click a[href="javascript:void(0);"]']='_handleBadLinkHref';_fieldProto.delegateEvents.call(this,events);},bindDomChange:function(){this.$(this.fieldTag).on('focus',_.bind(this.handleFocus,this));_fieldProto.bindDomChange.call(this);},handleFocus:function(evt){if(this.disposed){return;}
var left=this.$el.offset().left,right=this.$el.outerWidth()+left,top=this.$el.offset().top,bottom=this.$el.outerHeight()+top;this.view.trigger('field:focus:location',{left:left,right:right,top:top,bottom:bottom});},_handleBadLinkHref:function(evt){evt.preventDefault();}});});})(SUGAR.App);
...
(function(app){app.events.on('app:init',function(){app.lang=_.extend(app.lang,{getModuleSingular:function(module){var modString=app.metadata.getStrings('mod_strings')[module],moduleSingular=(modString?modString['LBL_MODULE_NAME_SINGULAR']:'')||app.lang.getAppListStrings('moduleListSingular')[module]||app.lang.getAppListStrings('moduleList')[module]||module;return moduleSingular;}});});app.events.on('app:sync:complete',function(){app.date.lang(app.user.getPreference('language'));});})(SUGAR.App);
