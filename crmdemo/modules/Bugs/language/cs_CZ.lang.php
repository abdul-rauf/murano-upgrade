<?php
/*
 * Your installation or use of this SugarCRM file is subject to the applicable
 * terms available at
 * http://support.sugarcrm.com/Resources/Master_Subscription_Agreements/.
 * If you do not agree to all of the applicable terms or do not have the
 * authority to bind the entity as an authorized representative, then do not
 * install or use this SugarCRM file.
 *
 * Copyright (C) SugarCRM Inc. All rights reserved.
 */

$mod_strings = array (
  // Dashboard Names
  'LBL_BUGS_LIST_DASHBOARD' => 'Řídicí panel seznamu chyb',
  'LBL_BUGS_RECORD_DASHBOARD' => 'Řídicí panel záznamů chyb',

  'LBL_MODULE_NAME' => 'Chyby',
  'LBL_MODULE_NAME_SINGULAR'	=> 'Chyba:',
  'LBL_MODULE_TITLE' => 'Sledování chyb',
  'LBL_MODULE_ID' => 'Chyby',
  'LBL_SEARCH_FORM_TITLE' => 'Hledat chyby',
  'LBL_LIST_FORM_TITLE' => 'Seznam chyb',
  'LBL_NEW_FORM_TITLE' => 'Nová chyba',
  'LBL_CONTACT_BUG_TITLE' => 'Kontakt-chyba:',
  'LBL_SUBJECT' => 'Předmět:',
  'LBL_BUG' => 'Chyba:',
  'LBL_BUG_NUMBER' => 'Číslo chyby:',
  'LBL_NUMBER' => 'Číslo:',
  'LBL_STATUS' => 'Stav:',
  'LBL_PRIORITY' => 'Priorita:',
  'LBL_DESCRIPTION' => 'Popis:',
  'LBL_CONTACT_NAME' => 'Jméno kontaktu:',
  'LBL_BUG_SUBJECT' => 'Chyba předmětu:',
  'LBL_CONTACT_ROLE' => 'Role:',
  'LBL_LIST_NUMBER' => 'Číslo',
  'LBL_LIST_SUBJECT' => 'Předmět',
  'LBL_LIST_STATUS' => 'Stav',
  'LBL_LIST_PRIORITY' => 'Priorita',
  'LBL_LIST_RELEASE' => 'Verze',
  'LBL_LIST_RESOLUTION' => 'Řešení',
  'LBL_LIST_LAST_MODIFIED' => 'Poslední modifikace',
  'LBL_INVITEE' => 'Kontakty',
  'LBL_TYPE' => 'Typ:',
  'LBL_LIST_TYPE' => 'Typ',
  'LBL_RESOLUTION' => 'Výsledek:',
  'LBL_RELEASE' => 'Nová verze:',
  'LNK_NEW_BUG' => 'Nahlásit chybu',
  'LNK_CREATE'  => 'Report chyb',
  'LNK_CREATE_WHEN_EMPTY'    => 'Reportovat chyby',
  'LNK_BUG_LIST' => 'Chyby',
  'LBL_SHOW_MORE' => 'Zobrazit více chyb',
  'NTC_REMOVE_INVITEE' => 'Opravdu chcete odstranit tento kontakt z chyb?',
  'NTC_REMOVE_ACCOUNT_CONFIRMATION' => 'Opravdu chcete vyjmout tuto chybu ze společnosti?',
  'ERR_DELETE_RECORD' => 'Pro odstranění chyby musíte specifikovat číslo záznamu.',
  'LBL_LIST_MY_BUGS' => 'Moje přidělené chyby',
  'LNK_IMPORT_BUGS' => 'Import chyb',
  'LBL_FOUND_IN_RELEASE' => 'Nalezeno ve verzi:',
  'LBL_FIXED_IN_RELEASE' => 'Opraveno ve verzi:',
  'LBL_LIST_FIXED_IN_RELEASE' => 'Opraveno ve verzi',
  'LBL_WORK_LOG' => 'Pracovní protokol:',
  'LBL_SOURCE' => 'Zdroj:',
  'LBL_PRODUCT_CATEGORY' => 'Kategorie:',

  'LBL_CREATED_BY' => 'Vytvořil:',
  'LBL_DATE_CREATED' => 'Vytvořeno, datum:',
  'LBL_MODIFIED_BY' => 'Naposled upravil:',
  'LBL_DATE_LAST_MODIFIED' => 'Datum úpravy:',

  'LBL_LIST_EMAIL_ADDRESS' => 'E-mailová adresa',
  'LBL_LIST_CONTACT_NAME' => 'Jméno kontaktu',
  'LBL_LIST_ACCOUNT_NAME' => 'Jméno společnosti',
  'LBL_LIST_PHONE' => 'Telefon',
  'NTC_DELETE_CONFIRMATION' => 'Opravdu chcete odstranit tento kontakt z chyb?',

  'LBL_DEFAULT_SUBPANEL_TITLE' => '"Bug Tracker"',
  'LBL_ACTIVITIES_SUBPANEL_TITLE'=>'Aktivity',
  'LBL_HISTORY_SUBPANEL_TITLE'=>'Historie',
  'LBL_CONTACTS_SUBPANEL_TITLE' => 'Kontakty',
  'LBL_ACCOUNTS_SUBPANEL_TITLE' => 'Společnosti',
  'LBL_CASES_SUBPANEL_TITLE' => 'Případy',
  'LBL_PROJECTS_SUBPANEL_TITLE' => 'Projekty',
  'LBL_DOCUMENTS_SUBPANEL_TITLE' => 'Dokumenty',
  'LBL_LIST_ASSIGNED_TO_NAME' => 'Zodpovědný uživatel',
	'LBL_ASSIGNED_TO_NAME' => 'Přiřazeno komu',

	'LNK_BUG_REPORTS' => 'Zobrazit chybová hlášení',
	'LBL_SHOW_IN_PORTAL' => 'Zobrazit na portálu',
	'LBL_BUG_INFORMATION' => 'Přehled úkolu',

    //For export labels
	'LBL_FOUND_IN_RELEASE_NAME' => 'Nalezeno v',
    'LBL_PORTAL_VIEWABLE' => 'Portal aktivní',
    'LBL_EXPORT_ASSIGNED_USER_NAME' => 'Přiřazený uživatel',
    'LBL_EXPORT_ASSIGNED_USER_ID' => 'ID přiřazeného uživatele',
    'LBL_EXPORT_FIXED_IN_RELEASE_NAMR' => 'Opraveno ve verzi',
    'LBL_EXPORT_MODIFIED_USER_ID' => 'ID modifikátora',
    'LBL_EXPORT_CREATED_BY' => 'Vytvořeno od ID:',


    'LBL_NOTES_SUBPANEL_TITLE' => 'Poznámky',

    // Help Text
    // List View Help Text
    'LBL_HELP_RECORDS' => 'Modul {{plural_module_name}} se používá ke sledování a správě problémů souvisejících s {{plural_module_name}}, obecně zmiňovaný jako chyby, jak nalezené interně nebo nahlášené zákazníky. {{plural_module_name}} mohou být dále vybrány sledováním vydání, ve kterém byly nalezeny a opraveny. Modul {{plural_module_name}} dává uživatelům způsob, jak rychle prohlédnout všechny detaily {{module_name}} a proces, použitý k opravě. Jakmile je {{module_name}} vytvořena a uložena, můžete si prohlédnout a upravit informace příslušející k {{module_name}} promocí pohledu na záznam {{module_name}}. Každý záznam {{module_name}} může souviset s jinými záznamy Sugaru, jako například {{calls_module}}, {{contacts_module}}, {{cases_module}}, a mnohé další.',

    // Record View Help Text
    'LBL_HELP_RECORD' => 'Modul {{plural_module_name}} se používá ke sledování a správě problémů souvisejících s produktem, obecně zmiňovaný jako {{plural_module_name}}, jak nalezené interně nebo nahlášené zákazníky.

- Upravte pole tohoto záznamu kliknutím na jednotlivé pole nebo tlačítko Upravit.
- Zobrazte nebo změňte vazby na ostatní záznamy v podřízených panelech přepnutím spodního pole na Data..
- Vytvořte a prohlédněte si uživatelské komentáře a historii změn záznamu v modulu {{activitystream_singular_module}} přepnutím spodního levého pole na Tok aktivity.
- Sledujte nebo označte tento záznam jako oblíbený pomocí ikon vpravo od názvu záznamu.
- Další akce jsou dostupné ve výběrovém menu Akce vpavo od talčítka Upravit.',

    // Create View Help Text
    'LBL_HELP_CREATE' => 'Modul {{plural_module_name}} se používá ke sledování a správě problémů souvisejících s produktem, obecně nazývaných {{plural_module_name}} nebo nedostatky, nalezených interně nebo nahlášených klienty.

Vytvoření modulu {{module_name}}:
1. Vyplňte hodnoty polí dle potřeby.
 - Pole označená jako „Povinné“ musí být vyplněna před uložením.
 - V případě potřeby klikněte na položku „Zobrazit více“ pro zobrazení dalších polí.
2. Kliknutím na tlačítko „Uložit“ dokončete nový záznam a vraťte se na předchozí stránku.',
);
