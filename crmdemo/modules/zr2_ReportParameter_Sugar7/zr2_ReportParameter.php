...
class zr2_ReportParameter extends SugarBean {
	...
	var $table_name = "zr2_reportparameter";
	var $object_name = "zr2_ReportParameter";
	...
	function mark_relationships_deleted($id) {
		$query = "UPDATE zr2_reportparameterlink set deleted=1 where parameter_id='$id'";
	...
	function fill_in_additional_detail_fields() {
		...
		if (array_key_exists($this->range_name, $app_list_strings['PARAM_RANGE_TYPES']))
			$this->range_description = $app_list_strings['PARAM_RANGE_TYPES'][$this->range_name];
	}
	...
	function get_parameter_html($rp, $rpl) {
		...
		$mod_strings = return_module_language($current_language, "zr2_ReportParameter");
		$mod_list_strings = return_mod_list_strings_language($current_language, "zr2_ReportParameter");
	
		$xtpl = new XTemplate('modules/zr2_ReportParameter/ParameterFill.html');
	...
	function get_sql_table($query = "", $limit = -1) {
		if (empty($query)) {			
			$query = html_entity_decode($this->range_options);		
		}
		if ($limit > 0) {
