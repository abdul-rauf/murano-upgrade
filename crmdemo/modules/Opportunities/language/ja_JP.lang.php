...
// BEGIN PRO/CORP
    // List View Help Text
    'LBL_HELP_RECORDS' => '{{plural_module_name}} モジュールを使用することで、個々の商談を開始から終了まで追跡できます。各 {{module_name}} レコードは見込み商談を表し、関連商談データおよび他の重要なレコード ({{quotes_module}}、{{contacts_module}} など) に関連するデータも含みます。{{module_name}} は通常、 いくつかの商談ステージを進み、最後に「受注」または「失注」とマークされます。{{plural_module_name}} は、Sugarの {{forecasts_singular_module}} を使用して商談の傾向を理解/予測したり、販売ノルマを達成する作業に集中したりすることで、さらに活用できます。',

    // Record View Help Text
    'LBL_HELP_RECORD' => '{{plural_module_name}} モジュールを使用することで、個々の商談と、それらの商談に
属している商品を開始から終了まで追跡できます。各 {{module_name}} レコードは見込み商談を表し、関連商談データおよび他の重要なレコード ({{quotes_module}}、{{contacts_module}} など) に関連するデータも含みます。

- このレコードのフィールドを編集するには、個々のフィールドまたは編集ボタンをクリックします。
- 他のレコードへのリンクをサブパネルで表示したり変更したりするには、下部の左ペインを「データビュー」に切り替えます。
- ユーザコメントやレコード変更履歴を {{activitystream_singular_module}} で作成したり表示したりするには、下部の左ペインを「アクティビティストリーム」に切り替えます。
- レコード名の右にあるアイコンを使用してこのレコードをフォローしたりお気に入りにしたりします。
- 編集ボタンの右にあるドロップダウンアクションメニューで追加アクションを利用できます。',

    // Create View Help Text
    'LBL_HELP_CREATE' => '{{plural_module_name}} モジュールを使用することで、個々の商談と、それらの商談に
属している商品を開始から終了まで追跡できます。各 {{module_name}} レコードは見込み商談を表し、関連商談データおよび他の重要なレコード ({{quotes_module}}、{{contacts_module}} など) に関連するデータも含みます。

{{module_name}} を作成するには:
1. 必要に応じてフィールドに値を入力します。
 - 「必須」とマークされたフィールドはすべて入力してから保存する必要があります。
 - 必要に応じて「更に表示」をクリックすると追加フィールドが表示されます。
